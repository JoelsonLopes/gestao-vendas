      ...
      <div>
        ...
        <div className="hidden print:block mt-4">
          <div className="print-header">
            ...
            <div className="flex justify-between items-start pb-6">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">Sistema especializado em gestão de pedidos para representantes</p>
                <p className="text-sm text-gray-500">Email: contato@gestaopedidos.com.br | Telefone: (51) 3333-4444</p>
              </div>
              
              <div className="text-right">
                <div className={`inline-flex items-center px-4 py-2 rounded-md font-bold text-white ${order.status === 'confirmado' ? 'bg-green-600' : 'bg-amber-500'}`}>
                  <span className="text-lg">{order.status === 'confirmado' ? 'PEDIDO CONFIRMADO' : 'COTAÇÃO'}</span>
                </div>
                <p className="mt-2 text-2xl font-bold text-gray-800">Nº {order.id}</p>
                <p className="text-sm text-gray-500">Data emissão: {formatDate(order.date)}</p>
              </div>
            </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="space-y-2">
                  <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                    ...
                    <span className="text-sm font-medium text-gray-800">{formatCurrency(order.subtotal)}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                    <span className="text-sm text-gray-600">Desconto:</span>
                    <span className="text-sm font-medium text-gray-800">{formatCurrency(order.discount)}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                    <span className="text-sm text-gray-600">Taxa de Frete:</span>
                    <span className="text-sm font-medium text-gray-800">{formatCurrency(order.taxes)}</span>
                  </div>
          ...
          <div className="mt-8 pt-4 border-t border-gray-300 text-center text-xs text-gray-500">
            <p>Este documento não possui valor fiscal. Gerado em {new Date().toLocaleDateString()} às {new Date().toLocaleTimeString()}</p>
            <p className="mt-1">Sistema de Gestão de Pedidos © {new Date().getFullYear()} - Todos os direitos reservados</p>
          </div>
        </div>
      </div>
