      ...
      <header className="w-full border-b border-blue-200 pb-4 mb-4">
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            {/* Badge de Status - Aumentado para maior visibilidade na impressão */}
            <div
              className="inline-block print-status rounded-md text-center"
              style={{
                borderColor:
                  order.status === "confirmado" ? "#16a34a" : "#f59e0b",
                backgroundColor:
                  order.status === "confirmado" ? "#16a34a" : "#f59e0b",
                color: "#ffffff",
                fontSize: "16px",
        ...
        <div className="flex w-full mb-4">
          <div className="flex-1 mr-4 p-4 bg-gray-50 rounded-md border border-gray-200">
            ...
            <p className="text-base font-medium">
              {order.clientName}{" "}
              <span className=""> - Cod. {order.clientCode}</span>
            </p>
          ...
          <div className="flex-1 p-4 bg-gray-50 rounded-md border border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              ...
              <h3 className="text-sm font-bold uppercase text-blue-600 border-b border-gray-200 pb-1 mb-2 text-right">
                STATUS
              </h3>

              <p className="text-base">{formatDate(order.date)}</p>
              <p className="text-base text-right">{order.status.toUpperCase()}</p>
            </div>
          </div>
            ...
            <tbody>
              {items.map((item, index) => {
                ...
                return (
                  ...
                  >
                    {showClientRefs && (
                      <td className="p-2 align-middle text-sm min-w-2">
                        {item.clientRef ? (
                          <span className="inline-block bg-blue-100 text-blue-700 px-2 py-1 rounded-md font-medium w-full">
                            {item.clientRef}
                          </span>
                        ) : (
                          "-"
                    ...
                    <td className="p-2 align-middle text-sm text-right font-bold">
        ...
        <section className="mt-8 mb-6">
          <div className="w-[50%] mx-auto">
            <div className="border-t border-gray-300 pt-2">
              ...
              <p className="text-xs text-center text-gray-500">
                Desenvolvido por Joelson Lopes Developer
              </p>
            </div>
          </div>
        </section>
